...
Task Interrupt calls `IsCurrentJobPlayerInterruptible` and
`EndCurrentJob(InterruptForced)` rather than replacing or patching them. Mods
that refine those methods therefore remain authoritative. Task Interrupt owns one
...
Task Interrupt does not patch the Controls dialog, poll global input, or add mouse
...
is a reason to keep Task Interrupt off those patch targets, not a claim that every
combination has been live-tested.

## Expected behavior

- Achtung-owned queued or forced work remains queued; Task Interrupt confirms the
